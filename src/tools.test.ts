...
describe('tools', () => {
  describe('getToolDefinitions', () => {
    it('returns only allowed tools', () => {
      const allowed = ['kognyt_sf_constraints', 'kognyt_sf_doc_search'];
      const tools = getToolDefinitions(allowed);
      expect(tools).toHaveLength(2);
      expect(tools.map(t => t.name)).toContain('kognyt_sf_constraints');
    });
    ...
    it('filters out disallowed tools', () => {
      const allowed = ['kognyt_sf_constraints'];
      const tools = getToolDefinitions(allowed);
      expect(tools).toHaveLength(1);
      expect(tools[0].name).toBe('kognyt_sf_constraints');
    });

    it('includes all free tier tools', () => {
      const freeTools = ['kognyt_sf_constraints', 'kognyt_sf_doc_search', 'kognyt_sf_releases'];
    ...
    it('includes pro tier tools when allowed', () => {
      const allTools = [
        'kognyt_sf_constraints', 'kognyt_sf_doc_search', 'kognyt_sf_releases',
        'kognyt_sf_rules', 'kognyt_sf_patterns', 'kognyt_sf_decision_guides'
      ];
  ...
  describe('handleToolCall', () => {
    ...
    it('returns null for disallowed tools', async () => {
      ...
      const result = await handleToolCall('kognyt_sf_rules', { query: 'test' }, mockClient, allowedTools);
      expect(result).toBeNull();
    });

    it('calls getConstraints for kognyt_sf_constraints', async () => {
      const allowedTools = new Set(['kognyt_sf_constraints']);
      await handleToolCall('kognyt_sf_constraints', { release_id: 'spring-26' }, mockClient, allowedTools);
      expect(mockClient.getConstraints).toHaveBeenCalledWith({ releaseId: 'spring-26' });
    });

    it('calls searchDocs for kognyt_sf_doc_search', async () => {
      const allowedTools = new Set(['kognyt_sf_doc_search']);
      await handleToolCall('kognyt_sf_doc_search', { query: 'apex' }, mockClient, allowedTools);
      expect(mockClient.searchDocs).toHaveBeenCalledWith({ query: 'apex' });
    });

    it('calls getReleases for kognyt_sf_releases', async () => {
      const allowedTools = new Set(['kognyt_sf_releases']);
      await handleToolCall('kognyt_sf_releases', { list_all: true }, mockClient, allowedTools);
    ...
    it('validates required query param for rules', async () => {
      const allowedTools = new Set(['kognyt_sf_rules']);
      const result = await handleToolCall('kognyt_sf_rules', {}, mockClient, allowedTools);
      expect(result?.isError).toBe(true);
      const textContent = result?.content.find((c: { type: string }) => c.type === 'text');
      expect((textContent as { text: string })?.text).toContain('query is required');
    });
    ...
    it('validates required query param for patterns', async () => {
      const allowedTools = new Set(['kognyt_sf_patterns']);
      const result = await handleToolCall('kognyt_sf_patterns', {}, mockClient, allowedTools);
      expect(result?.isError).toBe(true);
      const textContent = result?.content.find((c: { type: string }) => c.type === 'text');
      expect((textContent as { text: string })?.text).toContain('query is required');
    });
    ...
    it('validates required query param for decision_guides', async () => {
      const allowedTools = new Set(['kognyt_sf_decision_guides']);
      const result = await handleToolCall('kognyt_sf_decision_guides', {}, mockClient, allowedTools);
      expect(result?.isError).toBe(true);
      const textContent = result?.content.find((c: { type: string }) => c.type === 'text');
      expect((textContent as { text: string })?.text).toContain('query is required');
    });

    it('calls queryRules for kognyt_sf_rules', async () => {
      const allowedTools = new Set(['kognyt_sf_rules']);
      await handleToolCall('kognyt_sf_rules', { query: 'security' }, mockClient, allowedTools);
      expect(mockClient.queryRules).toHaveBeenCalledWith({ query: 'security' });
    });

    it('calls searchPatterns for kognyt_sf_patterns', async () => {
      const allowedTools = new Set(['kognyt_sf_patterns']);
      await handleToolCall('kognyt_sf_patterns', { query: 'batch' }, mockClient, allowedTools);
      expect(mockClient.searchPatterns).toHaveBeenCalledWith({ query: 'batch' });
    });

    it('calls searchDecisionGuides for kognyt_sf_decision_guides', async () => {
      const allowedTools = new Set(['kognyt_sf_decision_guides']);
      await handleToolCall('kognyt_sf_decision_guides', { query: 'architecture' }, mockClient, allowedTools);
